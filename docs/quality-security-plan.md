...
| GlitchTip 错误上报                    | 适配器、隐私过滤和发布流程已完成；待管理员配置项目与告警    |

当前缺口：

- Dependabot 告警和安全更新需要在 GitHub 仓库设置中启用；仓库配置只负责版本更新。
- GlitchTip 项目、仓库变量、Secret 和告警需要管理员在外部平台配置。
...
仓库侧实现已完成。外部项目、凭据和告警配置步骤见[错误监控与发布诊断](error-monitoring.md)；未配置时保持 disabled，不影响静态站点。
