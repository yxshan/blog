...
对 `21, -15, -15, -7, 15`，依次保留 `21`、`-15`、`-7`，删除第二个 `-15` 和最后的 `15`。
...
扫描到第一个 `-15` 时标记 `15`；再次扫描到 `-15` 或 `15` 时，发现 `seen[15]` 已经置为 `1`，通过前驱结点跳过当前结点。

## 代码实现

直接扫描前驱和标记表两套 C 代码已分别放在对应解法下；题目给出绝对值范围时优先使用标记表版本。
