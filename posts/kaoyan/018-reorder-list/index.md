...
链表 `a1->a2->a3->a4->a5` 重排后为 `a1->a5->a2->a4->a3`；偶数长度时 `a1->a2->a3->a4` 重排为 `a1->a4->a2->a3`。
...
以五个结点为例，慢指针找到 `a3`，将链表拆成 `a1->a2->a3` 和 `a4->a5`；逆置后半段得到 `a5->a4`，交替合并得到目标顺序。

## 代码实现

数组基线解和原地重排两套 C 代码已分别放在对应解法下；题目要求 `O(1)` 额外空间时使用原地版本。
