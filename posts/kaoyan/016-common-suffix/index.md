...
若两个链表分别为 `a->b->i->n->g` 和 `b->e->i->n->g`，其中 `i` 之后的结点实际共享，则公共后缀起点是结点 `i`。
...
先求两个链表长度和尾结点。若尾结点相同，再让较长链表的指针先走过长度差；此后两个指针同步前进，第一次指针相等的位置就是公共后缀起点。

## 代码实现

直接地址枚举和长度对齐两套 C 代码已分别放在对应解法下；实际答题优先使用长度对齐版本。
